...
export function BookPage() {
  ...
  if (!slug) return <Navigate to={routes.BOOKS()} />
    ...
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.breadcrumb}>
          ...
          <div className={styles.current}>{book?.name}</div>
        </div>
      </div>
      <div className={styles.imageAndCards}>
        {!isLoading && <img className={styles.imageCard} src={book?.bannerImageUrl ?? '/assets/images/book.svg'} />}
        <div className={styles.cards}>
          <div className={styles.infosCard}>
            <div className={styles.title}>{book?.name}</div>
            <div className={styles.available}>({book?.rented ? 'Indisponível' : 'Disponível'} para aluguel)</div>
            <div className={styles.info}>{book?.authorName}</div>
            <div className={styles.info}>{book?.editorName}</div>
            <div className={styles.info}>Publicação: {book?.publishedYear}</div>
      ...
      <div className={styles.detailsCard}>
        <div className={styles.title}>Detalhes</div>
        <div>Nome: {book?.name}</div>
        <div>Autor: {book?.authorName}</div>
        <div>Editora: {book?.editorName}</div>
        <div>Ano de publicação: {book?.publishedYear}</div>
        <div>Idioma: {book?.language?.name}</div>
        <div>Páginas: {book?.pageCount}</div>
        <div>Gêneros: {book?.genres.map((genre) => genre.name).join(', ')}</div>
      </div>
    </div>
  )
}
