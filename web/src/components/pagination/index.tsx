      ...
      <div className={styles.itemsPerPage}>
        ...
        <div className={styles.buttons}>
          {pageSizes.map((item, i) => (
            <button key={i} className={pageSize === item ? styles.selected : ''} onClick={() => onPageSizeChange?.(item)}>
              {item}
            </button>
          ))}
        </div>
      ...
      <div className={styles.pages}>
        ...
        <div className={styles.buttons}>
          {showingPages.map((item, i) => (
            <button key={i} className={page === item ? styles.selected : ''} onClick={() => onPageChange?.(item)}>
              {item + 1}
            </button>
          ))}
        </div>
